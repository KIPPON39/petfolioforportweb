...
export default function EditPostPage() {
  ...
  useEffect(() => {
    ...
    // ดึงข้อมูลโพสต์
    fetch(`https://petfolioforportweb.onrender.com/api/community-posts/communityposts/${postId}`, {
    ...
    if (!userId) return;
    fetch(`https://petfolioforportweb.onrender.com/api/pets/user/${userId}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
      .then(res => res.json())
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `https://petfolioforportweb.onrender.com/api/community-posts/updatePost/${postId}`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
