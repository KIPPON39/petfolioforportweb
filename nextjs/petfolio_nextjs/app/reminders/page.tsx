...
export default function Reminder() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch Pets
        const petsRes = await fetch(`https://petfolio.wisitdev.com/api/pets/user/${userId}`, {
        ...
        // Fetch Reminders
        const remRes = await fetch(`https://petfolio.wisitdev.com/api/reminders/user/${userId}`, {
          headers: token ? { Authorization: `Bearer ${token}` } : {},
        });
  ...
  const handleMarkComplete = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`https://petfolio.wisitdev.com/api/reminders/${id}/complete`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
